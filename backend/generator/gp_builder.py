...
class GPBuilder:
    ...
    def _normalize_zouit_name(self, name: str) -> str:
        ...
        - заменяет "похожую" латиницу на кириллицу (C→С, A→А, E→Е и т.п.),
          чтобы обрабатывать кривые наименования из реестров;
        - убирает типовые префиксы ("зона с особыми условиями использования территории",
          "зоуит", "охранная зона" и т.п.);
        - чистит кавычки и лишние пробелы.
        """
        if not name:
            return ""
    ...
    def get_zouit_block_filename(self, zouit_name: str) -> Optional[str]:
        ...
        Приоритет:
        1) Специальные "жёсткие" правила HIGH_PRIORITY_ZOUIT_RULES по подстрокам
           исходного названия (с фиксом латиницы → кириллица).
        2) Обобщённый скоринговый поиск по ZOUIT_MAPPING (как раньше).
        """
        if not zouit_name:
            return None
