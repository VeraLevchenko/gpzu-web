...
ОБНОВЛЕННАЯ ВЕРСИЯ с правильными названиями файлов ЗОУИТ
"""
...
ZOUIT_MAPPING = {
    ...
    "64_aeroport_podzona5.docx": [
        "пят подзон", "пятая подзона", "5 подзона", "пятой подзоны", "подзона 5"
    ],
    "64_aeroport_podzona6.docx": [
        "шест подзон", "шестая подзона", "6 подзона", "шестой подзоны", "подзона 6"
    ],
...
class GPBuilder:
    """
    ОБНОВЛЕННЫЙ генератор градостроительного плана земельного участка (ГПЗУ).
    С правильными названиями файлов ЗОУИТ.
    """
    ...
    def _normalize_zouit_name(self, name: str) -> str:
        ...
        # Убираем лишние символы и кавычки
        normalized = re.sub(r'[«»""\'\"]+', ' ', normalized)
        normalized = re.sub(r'\s+', ' ', normalized)
        
        # Убираем префиксы типа "зона с особыми условиями использования территории"
        prefixes_to_remove = [
    ...
    def get_zouit_block_filename(self, zouit_name: str) -> Optional[str]:
        """
        ОБНОВЛЕННАЯ функция определения имени файла блока ЗОУИТ по наименованию.
        
        Использует реальные названия файлов из директории zouit_reglament.
        """
        if not zouit_name:
            return None
        ...
        if best_match and best_score > 0:
            logger.info(f"✅ Найдено совпадение для ЗОУИТ '{zouit_name}': {best_match} (score: {best_score:.2f})")
            return best_match
        else:
    ...
    def test_zouit_matching(self, test_names: List[str]) -> Dict[str, str]:
        """
        Тестирует сопоставление названий ЗОУИТ с файлами.
        
        Args:
            test_names: Список названий для тестирования
            
        Returns:
            Словарь {название: найденный_файл}
        """
        results = {}
        
        logger.info("=== ТЕСТИРОВАНИЕ СОПОСТАВЛЕНИЯ ЗОУИТ ===")
        
        for name in test_names:
            filename = self.get_zouit_block_filename(name)
            results[name] = filename or "НЕ НАЙДЕН"
    ...
    def fill_zouit_table(self, doc: Document, zouit_list: List[Dict[str, Any]]) -> None:
        ...
        Требования:
        - сохраняем шапку таблицы и строку с номерами столбцов «1 2 3 4»;
        - добавляем строки только ниже шапки;
        - выводим в первой колонке: "<наименование> (<реестровый номер>)";
        - остальные колонки (координаты) оставляем пустыми;
        - сохраняем границы таблицы как в шаблоне (стиль Table Grid, если доступен).
        """
        if not zouit_list:
    ...
    def insert_zouit_blocks(self, doc: Document, zouit_list: List[Dict[str, Any]]) -> None:
        ...
        ОБНОВЛЕННАЯ ВЕРСИЯ с правильными именами файлов.
        """
